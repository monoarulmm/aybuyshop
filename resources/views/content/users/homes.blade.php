    ...
    <div class="space-y-10 pb-20">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            @forelse ($videos as $index => $video)
                {{-- ৯টি ভিডিওর লিমিট চেক (লগইন ইউজারদের জন্য) --}}
                @if ($index < 9)
                    <div id="video-card-{{ $video->id }}"
                        ...
                        <div class="aspect-video w-full relative bg-black flex items-center justify-center overflow-hidden">
                            ...
                            @if ($video->video_type == 'facebook')
                                <div class="fb-wrapper w-full h-full relative" id="fb-container-{{ $video->id }}">
                                    <div id="fb-player-{{ $video->id }}" class="fb-video"
                                        data-href="{{ $video->video_url }}" data-width="auto" data-allowfullscreen="true"
                                    ...
                                    <div id="fb-starter-{{ $video->id }}"
                                        ...
                                        <p
                                            class="text-[10px] text-gray-500 mt-4 uppercase font-black tracking-widest italic">
                            ...
                            <div id="msg-overlay-{{ $video->id }}"
                                ...
                                <p id="status-text-{{ $video->id }}" class="text-gray-400 text-xs mt-1 mb-6 max-w-xs">
                                </p>
        ...
        {{-- ৯টির বেশি ভিডিও থাকলে 'See More' বাটন --}}
        @if ($videos->count() > 9)
            <div class="flex justify-center pt-6">
                <button
                    class="bg-gray-800 hover:bg-gray-700 text-white px-8 py-3 rounded-2xl font-bold border border-gray-700 transition-all">
                    আরো ভিডিও দেখুন ➔
                </button>
            </div>
        @endif

        {{-- লগইন ছাড়া ইউজারদের জন্য ফিক্সড ৩টি ভিডিওর নিচে মেসেজ --}}
        @guest
            <div class="bg-blue-600/10 border border-blue-500/20 p-6 rounded-[2rem] text-center">
                <h3 class="text-white font-bold mb-2">আরো ভিডিও দেখে আয় করতে চান?</h3>
